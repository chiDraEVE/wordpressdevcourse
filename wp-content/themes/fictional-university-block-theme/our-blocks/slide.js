	...
	useEffect(function () {
		if (props.attributes.imgID) {
			async function go() {
				...
				const imgSizes = response.media_details.sizes
				if (imgSizes.pageBanner)
					props.setAttributes({imgURL: imgSizes.pageBanner.source_url})
				else if (imgSizes.full)
					props.setAttributes({imgURL: imgSizes.full.source_url})
				else
					props.setAttributes({imgURL: response.media_details.file})

				props.setAttributes({ themeimage: ""})
			}
			go()
		}
